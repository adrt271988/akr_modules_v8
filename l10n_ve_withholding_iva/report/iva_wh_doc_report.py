...
class report_iva_wh_doc(report_sxw.rml_parse):

    def __init__(self, cr, uid, name, context=None):
        super(report_iva_wh_doc, self).__init__(cr, uid, name, context=context)
    ...
    def _get_iva(self,invoice_id):
        for i in self.pool.get('account.invoice').browse(self.cr,self.uid,invoice_id).tax_line:
            if not i.tax_id.ret:
                return "{0:.2f}".format(i.tax_id.amount*100)
            else:
                return 0
...
class reportIvaWhDoc(osv.AbstractModel):
    _name = 'report.l10n_ve_withholding_iva.withholding_vat_report'
    _inherit = 'report.abstract_report'
    _template = 'l10n_ve_withholding_iva.withholding_vat_report'
    _wrapped_report_class = report_iva_wh_doc
